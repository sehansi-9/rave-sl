...
const AddEvent = ({ map }) => {
    ...
    const postDetails = async () => {
        const token = user && await user.getIdToken(); // checks if the user is logged in. if user is, gets their ID token
        const headers = token ? {"Content-Type": "application/json", authtoken: token} : {}; // if the token exists authtoken is set to that or an empty field if it doesn't

        if (!user) {
            M.toast({html: "Whoops! You need to be logged in to add your event!", classes: "blue darken-2"})
        }
    ...
    // adding a function to blur the input fields if they are not logged in
    const blur = (event)=>{
        if (!user){
            event.target.blur();
        }
    ...
    return (
        <div>
            {user ? (
                ...
                <div>You need to Login to Create an Event</div>
            )}
        </div>
    );
